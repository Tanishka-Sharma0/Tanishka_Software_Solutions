                        ...
                        <div className="overflow-y-auto h-full">
                            {conversations.map((conv) => (
                                <div key={conv._id}
                                    ...
                                    className={`p-4 cursor-pointer hover:bg-gray-50 ${selectedUser?._id === conv._id ? 'bg-green-50' : ''}`}>
                        ...
                        {selectedUser ? (
                            <>
                                ...
                                <div className="flex-1 overflow-y-auto p-4 space-y-4">
                                    {messages.map((msg) => (
                                        <div key={msg._id} className={`flex ${msg.sender._id === user._id ? 'justify-end' : 'justify-start'}`}>
                                            <div className={`max-w-xs p-3 rounded-lg ${msg.sender._id === user._id ? 'bg-green-600 text-white' : 'bg-gray-100'}`}>
                                                <p>{msg.content}</p>
                                ...
                                <div className="p-4 border-t flex">
                                    ...
                                    <button onClick={sendMessage} className="bg-green-600 text-white px-4 rounded-r-lg">
                                        <FiSend />
                                    </button>
                                </div>
                            </>
